...
def test_group_exact_score():
    assert score_prediction(2, 1, 2, 1, "group", None, None, CONFIG) == CONFIG["exact_score"]


def test_group_correct_result_home_win():
    assert score_prediction(3, 0, 2, 1, "group", None, None, CONFIG) == CONFIG["correct_result"]


def test_group_correct_result_draw():
    assert score_prediction(1, 1, 0, 0, "group", None, None, CONFIG) == CONFIG["correct_result"]


def test_group_correct_result_away_win():
    assert score_prediction(0, 2, 0, 1, "group", None, None, CONFIG) == CONFIG["correct_result"]
...
def test_knockout_exact_score():
    assert score_prediction(1, 0, 1, 0, "r16", None, None, CONFIG) == CONFIG["exact_score"]


def test_knockout_correct_result():
    assert score_prediction(2, 0, 3, 1, "r16", None, None, CONFIG) == CONFIG["correct_result"]
...
def test_knockout_pens_winner_correct():
    ...
    assert pts == CONFIG["exact_score"] + CONFIG["pens_winner"]


def test_knockout_pens_winner_wrong():
    pts = score_prediction(1, 1, 1, 1, "r16", 10, 20, CONFIG)
    assert pts == CONFIG["exact_score"]  # exact score but wrong pens winner
...
def test_knockout_correct_result_with_pens_winner():
    ...
    assert pts == CONFIG["correct_result"] + CONFIG["pens_winner"]
